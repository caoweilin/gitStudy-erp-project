...
<body>
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        ...
	        <h4 class="modal-title" id="myModalLabel">录用信息</h4>
	      </div>
	      <div class="modal-body" id="modal_train_id">
	      	<form class="form-horizontal" role="form" method="post" id="form_offer_id">
	      		<div class="form-group">
			    <label for="inputEmail3" class="col-sm-2 control-label">录用薪资</label>
			    <div class="col-sm-6">
			      <input type="hidden" class="form-control" name="userId" id="user_id_id">
			      <input type="hidden" class="form-control" name="resumeid" id="resume_id">
			      <input type="text"  class="form-control" name="salary" id="salary_id">
			    </div>
			  </div>
			  <div class="form-group">
			   <label for="inputEmail3" class="col-sm-2 control-label">录用职位</label>
			   <div class="col-sm-4">
			   	   <label  >部门</label>
				   <select class="form-control" id="select_offer_dept" name="jobDept">
				   	<option value=0></option>
					</select>
				</div>
				<div class="col-sm-4">
					<label>职位</label>
					<select class="form-control" id="select_offer_post" name="parentId">
					</select>
				</div>
			  </div>
	      	</form>
		...
		function fill_resume_data(list) {
			$.each(list, function(index, item) {
				...
				if(item.state == 6 || item.state == 7) {
					var offerBtn = $("<button></button>").addClass("btn btn-default btn-sm btn_offer")
					.append($("<span></span>").addClass("glyphicon glyphicon-ok-sign")).append("录用");
					offerBtn.attr("resume_id", item.resumeId).attr("resume_name", item.name)
					.attr("userId", item.userId).attr("disabled", true);
					var offerTd = $("<td></td>").append(offerBtn)
				}else {
					var offerBtn = $("<button></button>").addClass("btn btn-success btn-sm btn_offer")
					.append($("<span></span>").addClass("glyphicon glyphicon-ok-sign")).append("录用");
					offerBtn.attr("resume_id", item.resumeId).attr("resume_name", item.name).attr("userId", item.userId);
					var offerTd = $("<td></td>").append(offerBtn)
				}
				//拒绝操作
				if(item.state == 6 || item.state == 7) {
					var refuseBtn = $("<button></button>").addClass("btn btn-default btn-sm btn_refuse")
					.append($("<span></span>").addClass("glyphicon glyphicon-remove-sign")).append("不录用");
					refuseBtn.attr("resume_id", item.resumeId).attr("resume_name", item.name).attr("disabled", true);
					var refuseTd = $("<td></td>").append(refuseBtn)
				}else {
					var refuseBtn = $("<button></button>").addClass("btn btn-danger btn-sm btn_refuse")
					.append($("<span></span>").addClass("glyphicon glyphicon-remove-sign")).append("不录用");
					refuseBtn.attr("resume_id", item.resumeId).attr("resume_name", item.name);
					var refuseTd = $("<td></td>").append(refuseBtn)
				}
	...
	//=======================================录用==================================================
	$(document).on("click", ".btn_offer", function() {
		//填充隐藏简历id
		$("#resume_id").val($(this).attr("resume_id"));
		//填充隐藏的用户id
		$("#user_id_id").val($(this).attr("userId"));
		//给确认录用按钮添加名字
		$(".btn_offer_sure").attr("resume_name", $(this).attr("resume_name"));
		//填充职位信息
		geDept_offer_data();
		//弹出录用模态框
		$("#modal_offer").modal({
	...
	$(".btn_offer_sure").click(function() {
		...
		var parentId = $("#select_offer_post").val();
		console.log(parentId);
		if( salary != '') {
			if(parentId != '' && parentId != null) {
				if(confirm("再次确认是否录用"+$(this).attr("resume_name"))) {
				$.ajax({
					url : "${APP_PATH}/resume/offerVisitorResume",
	...
	//给简历更新状态  update_resume_check($(this).attr("resume_id"), state);
	...
	//=======================================不录用================================================
		$(document).on("click", ".btn_refuse", function() {
			//确认不录用
			if(confirm("确认不录用"+$(this).attr("resume_name") +"吗？")) {
				$.ajax({
					url : "${APP_PATH}/resume/refuseVisitorResume/"+$(this).attr("resume_id"),
					success : function(data) {
						if(data) {
							alert("操作成功！");
							to_page(currentpage);
