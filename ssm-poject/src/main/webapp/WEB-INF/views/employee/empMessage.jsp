...
<body>
	...
	<script type="text/javascript">
	 ...
	 function message_handle() {
		 ...
		 var trainMessage = ${trainMessage}
		 var trainStartMessage = ${trainStartMessage}
		 if(appointMessage > 0) {
			 //填充提示信息
			 var message = $("<h4></h4>")
				.append("您有"+appointMessage+"条委派面试消息!")
		 ...
		 if(trainStartMessage > 0) {
			 //填充提示信息
			 var message = $("<h4></h4>")
				.append("您有"+trainStartMessage+"条培训正在进行，请前去培训!")
				.appendTo("#modal_train_id")
			 //弹出模态框
			 $("#message_modal").modal({
				 backdrop : "static"
		...
		function to_page(pn) {
			$.ajax({
				url : "${APP_PATH}/newMessage/getEmpMessageData/"+pn,
				type : "GET",
				success : function(e) {
		...
		function table_data(e) {
			...
			$.each(data, function(index, item) {
				...
				var appointedtimeTd = $("<td></td>").append(item.appointedtime);
