...
<body>
	      ...
	      <div class="modal-body">
	        <form class="form-horizontal" method="post" id="update_wage_form">
			  <div class="form-group">
			   <label for="inputEmail3" class="col-sm-2 control-label">审批内容</label>
			   <div class="col-sm-6">
				   <textarea type="text" class="form-control" name="adminDescript"  id="reason_add_id"></textarea>
				</div>
			  </div>
			</form>
	      </div>
			...
			<div class="col-md-10">
				<div class="panel panel-default">
				    <div class="panel-heading">
						...
						<button type="button" class="btn btn-success" id="btn_allwage" data-toggle="tooltip" data-placement="top" title="模拟结算">薪资结算</button>
				    </div>
	...
	//给查询按钮添加年份月份
	 function fill_check_year() {
		...
		function to_page(pn) {
			$.ajax({
				url : "${APP_PATH}/wage/getReConsiderWage/"+pn,
				type : "GET",
				success : function(e) {
					if(e) {
		...
		function table_data(e) {
			...
			$.each(data, function(index, item) {
				...
				if(item.state == 1) {
					...
					.append($("<span></span>").addClass("glyphicon glyphicon-tags")).append(" ").append("审批");
					...
					//复议按钮
					var reConsiderBtn = $("<button></button>").addClass("btn btn-danger btn-sm btn_wage_reConsider")
					.append($("<span></span>").addClass("glyphicon glyphicon-tags")).append(" ").append("已审批");
				...
				.append(nameTd).append(jobPositionTd).append(baseWageTd).append(perfWageTd)
				.append(ovWageTd).append(rapWageTd).append(jinpoWageTd)
				.append(totalWageTd).append(descriptTd).append(chanstate).append(considerTd)
		...
		$("#add_reConsider_sure").click(function() {
			...
			$.ajax({
				url : "${APP_PATH}/wage/updateAdminWage/"+$(this).attr("wage_id"),
	...
	//=====================================薪资结算======================================================
	$("#btn_allwage").click(function() {
		//请求结算工资
		$.ajax({
			url : "${APP_PATH}/wage/wageSettlement",
			success : function(data) {
				if(data) {
					console.log("^-^")
					console.log(data);
